"""
patch_creator.py
Create patches from slides/ annotated slides.
"""
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--source_dir", type=str, help="Path to folder containing the image files"
    )
    parser.add_argument(
        "--patch_dir", type=str, help="Path to folder for storing the patches"
    )
    parser.add_argument(
        "--function_type",
        type=str,
    ...
    parser.add_argument(
        "--thresholds", type=dict, help="Threshold to consider for the annotated images"
    )
...
def tile(filename, dir_in, dir_out, d):
    ...
    grid = product(range(0, h - d + 1, d), range(0, w - d + 1, d))
    for i, j in grid:
        box = (j, i, j + d, i + d)
        i /= d
        j /= d
...
def tile_annotations(filename, dir_in, dir_out, d):
    ...
    ret, img_cv_binarized = cv2.threshold(
        img_cv_gray, thresholds["annotations"], 255, cv2.THRESH_TOZERO
    )  # Apply thresholding
    img_pil_binarized = cv2.cvtColor(
        img_cv_binarized, cv2.COLOR_BGR2RGB
    ...
    save_object = {"patch_scores": patch_scores, "bin_scores": bin_scores}
...
def artefact_annotations(filename, dir_in, dir_out, d):
    ...
    # Thresholding options: ['THRESH_BINARY', 'THRESH_BINARY_INV', 'THRESH_TOZERO ', 'THRESH_TOZERO_INV', 'THRESH_OTSU']
    ret, img_cv_binarized = cv2.threshold(
        img_cv_gray, thresholds["artefacts"], 255, cv2.THRESH_BINARY_INV
    )  # Apply thresholding
    img_pil_binarized = cv2.cvtColor(
        img_cv_binarized, cv2.COLOR_BGR2RGB
    )  # Convert to RGB, for PIL Image
...
for filename in os.listdir(source_dir):
    ...
    if function_type == "tile":
        ...
        tile_annotations_overlap(
            filename, source_dir, output_patches_dir, patch_size, overlap
        )
