...
def compute_from_patches_overlap(
    clam_pred=None,
    model=None,
    feature_extractor=None,
    batch_size=512,
...
def generate_heatmap_dict(exp_code, use_overlap=True):
    ...
    for split in tqdm(splits, desc="Splits", unit="split", leave=False):
        ...
        ckpt_path = os.path.join(
            results_dir, exp_code, "splits_" + str(split), ckpt_path
        )
        ...
        if use_overlap:
            heatmap_dict = compute_from_patches_overlap(
                model=model,
...
def draw_heatmaps(exp_code, cmap="coolwarm"):
    ...
    for split in splits:
        ...
        for image_file in heatmap_dict:
            ...
            for index, block_score in enumerate(percentiles):
                ...
                if index % 4 == 0:
                    plt.text(
                        y + 0.5 * patch_size[1],
                        x + 0.5 * patch_size[0],
                        str(round(percentiles[index], 4))
                        + "\n"
                        + str(round(scores[index], 4)),
                        fontsize="x-small",
                    )

            heatmap_mask = cv.blur(heatmap_mask, tuple(blur))

            img_heatmap_filename = os.path.join(
                save_path, image_name + "_heatmap" + ".png"
            )
            ...
            img_heatmap = cv.addWeighted(
                orig_img, alpha, heatmap_mask, beta, gamma, dtype=cv.CV_64F
            )
...
def draw_heatmaps_overlap(exp_code, cmap="coolwarm"):
    ...
    for split in splits:
        ...
        for image_file in heatmap_dict:
            ...
            if get_patch_acc:
                patch_annot_path = os.path.join(
                    annot_dir, image_name, image_name + ".pkl"
                )
                if "F" in image_name:
            ...
            for index, block_score in enumerate(percentiles):
                ...
                if show_labels:
                    plt.text(
                        y + 0.5 * patch_size[1],
                        x + 0.5 * patch_size[0],
                        str(math.floor(percentiles[index] * 100))
                        + "\n"
                        + str(math.floor(scores[index] * 1e4)),
                        fontsize="x-small",
                    )
        ...
        if get_patch_acc:
            ...
            acc_dict_save = os.path.join(
                results_dir, exp_code, "splits_" + str(split), "acc_dict.pkl"
            )
