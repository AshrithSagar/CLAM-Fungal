...
dataset = Generic_WSI_Classification_Dataset(
    csv_path=os.path.abspath(dataset_csv),
    shuffle=False,
    seed=seed,
    print_info=True,
...
for lf in label_fracs:
    split_dir = os.path.join(
        split_dir, "fungal_vs_nonfungal" + "_{}".format(int(lf * 100))
    )
    print(split_dir)
    ...
    for i in range(k):
        ...
        save_splits(
            splits,
            ["train", "annot", "val", "test"],
            os.path.join(split_dir, "splits_{}.csv".format(i)),
            annot_frac=annot_frac,
            annot_positive_frac=annot_positive_frac,
        )
