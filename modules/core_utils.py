...
def train(datasets, cur, settings, test_metrics):
    ...
    exp_dir = os.path.join(
        ...
        str(settings["exp_code"]) + "_s{}".format(settings["seed"]),
    )
    ...
    save_splits(
        datasets,
        ["train", "val", "test"],
        os.path.join(split_dir, "splits_{}.csv".format(cur)),
        annot_create=False,
    )
    print("Done!")
        ...
        if settings["model_type"] == "clam_sb":
            model = CLAM_SB(
                **model_dict,
                ...
                attention_labels_loss_fn=attention_labels_loss_fn,
            )
    ...
    train_loader = get_split_loader(
        ...
        weighted=settings["weighted_sample"],
    )
    ...
    for epoch in range(settings["max_epochs"]):
        ...
        if (
            settings["model_type"] in ["clam_sb", "clam_mb"]
            and not settings["no_inst_cluster"]
        ):
            train_loop_clam(
                epoch,
                model,
    ...
    if settings["early_stopping"]:
        model.load_state_dict(
            torch.load(os.path.join(split_dir, "s_{}_checkpoint.pt".format(cur)))
        )
    else:
        torch.save(
            model.state_dict(),
            os.path.join(split_dir, "s_{}_checkpoint.pt".format(cur)),
        )

    pd.DataFrame(train_metrics).to_csv(
        os.path.join(split_dir, f"train_metrics_fold_{cur}.csv"), index=False
    )
    pd.DataFrame(valid_metrics).to_csv(
        os.path.join(split_dir, f"valid_metrics_fold_{cur}.csv"), index=False
    )
...
def train_loop_clam(
    ...
    train_inst_loss = 0.0
    inst_count = 0
    labeled_count = 0.0  # Labelled images
    train_attention_labels_loss = 0.0
    ...
    for batch_idx, (data, label, idx, bool_annot, patch_annot) in enumerate(loader):
        ...
        logits, Y_prob, Y_hat, _, instance_dict, attention_labels_loss = model(
            data,
            label=label,
            alpha_weight=alpha_weight,
            semi_supervised=semi_supervised,
            bool_annot=bool_annot,
        ...
        total_loss = (
            loss_weights["bag"] * loss + loss_weights["instance"] * instance_loss
        )
        ...
        if attention_labels_loss is not None:
            attention_labels_loss_value = (
                loss_weights["attention_labels"] * attention_labels_loss
            )
            total_loss += attention_labels_loss_value
            train_attention_labels_loss += attention_labels_loss_value
            labeled_count += 1
        else:
            attention_labels_loss = -1.0
        ...
        if (batch_idx + 1) % 20 == 0:
            print(
                "batch {}, loss: {:.4f}, instance_loss: {:.4f}, attention_labels_loss: {:.4f}, weighted_loss: {:.4f}, ".format(
                    batch_idx,
                    loss_value,
                    instance_loss_value,
                    attention_labels_loss,
                    total_loss.item(),
                )
    ...
    if writer:
        ...
        if labeled_count != 0:
            writer.add_scalar(
                "train/attention_labels_loss", train_attention_labels_loss, epoch
            )
        ...
        writer.add_scalar(
            "train/inst_specificity", inst_logger.get_specificity(), epoch
        )
...
def validate(
    ...
    if early_stopping:
        ...
        early_stopping(
            ...
            val_loss,
            model,
            ckpt_name=os.path.join(split_dir, "s_{}_checkpoint.pt".format(cur)),
        )
