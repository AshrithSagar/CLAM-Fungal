...
class CLAM_SB(nn.Module):
    ...
    def inst_eval(self, A, h, classifier, bool_annot, patch_annot, weight_alpha):
        device=h.device
        if len(A.shape) == 1:
            A = A.view(1, -1)
...
#         print("p_targets", p_targets.shape)
    ...
    def forward(self, h, bool_annot=None, patch_annot=None, weight_alpha=None, label=None, instance_eval=False, return_features=False, attention_only=False):
        device = h.device
...
#                 print("inst_label", inst_label)
                ...
                if inst_label == 1: #in-the-class:
                    instance_loss, preds, targets = self.inst_eval(A, h, classifier, bool_annot, patch_annot, weight_alpha)
