...
class CLAM_SB(nn.Module):
    ...
    def inst_eval(
        ...
        if semi_supervised and bool_annot:
            ...
            if label:
                ...
                positive_preds = torch.tensor(
                    [A.view(self.num_patches)[idx].item() for idx in postive_patch_ids],
                    device=device,
                )
                negative_preds = torch.tensor(
                    [
                        A.view(self.num_patches)[idx].item()
                        for idx in negative_patch_ids
                    ],
                    device=device,
                )

                pos_targets_L1 = torch.full((len(postive_patch_ids),), 1, device=device)
                neg_targets_L1 = torch.full(
                    (len(negative_patch_ids),), 0, device=device
                )
    ...
    def forward(
        ...
        if semi_supervised and bool_annot:
            A_attention_preds = A.view([self.num_patches])
            attention_labels_loss = self.attention_labels_loss_fn(
                A_attention_preds, targets.float()
            )
        else:
            attention_labels_loss = None

        M = torch.mm(A.view(1, self.num_patches), h.view(self.num_patches, 512))
            ...
            for i in range(len(self.instance_classifiers)):
                ...
                if inst_label == 1:  # in-the-class:
                    instance_loss, preds, targets = self.inst_eval(
                        A.view(self.n_classes, 24)[i], h, classifier
                    )
