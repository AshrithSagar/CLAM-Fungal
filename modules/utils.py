...
def collate_MIL_mod(batch):
    img = torch.cat([item[0] for item in batch], dim = 0)
    label = torch.LongTensor([item[1] for item in batch])
    idx = torch.LongTensor([item[2] for item in batch])
    bool_annot = torch.LongTensor([item[3] for item in batch])
    patch_annot = torch.LongTensor([item[4] for item in batch])
    return [img, label, idx, bool_annot, patch_annot]
...
def collate_MIL_annot(batch):
    ...
    bool_annot = torch.LongTensor([item[3] for item in batch])
    patch_annot = torch.LongTensor([item[4] for item in batch])
    return [img, label, slide_id, bool_annot, patch_annot]
    ...
    if not testing:
        if training:
            if weighted:
                ...
                loader = DataLoader(split_dataset, batch_size=1, sampler = RandomSampler(split_dataset), collate_fn = collate_MIL_mod, **kwargs)
        else:
