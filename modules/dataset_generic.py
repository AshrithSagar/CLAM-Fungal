...
def save_splits(
    ...
    if annot_create:
        ...
        neg_annot_num = np.round(train_set_annot * (1 - annot_positive_frac)).astype(
            int
        )
    ...
    if not boolean_style:
        ...
        df = pd.DataFrame(
            bool_array, index=index, columns=["train", "annot", "val", "test"]
        )
...
class Generic_WSI_Classification_Dataset(Dataset):
    ...
    def get_overlap_split_from_df(
        self,
        all_splits,
        split_keys=["train", "annot"],
        training=False,
    ...
    def return_splits(self, from_id=True, csv_path=None, use_augmentation=True):
        if from_id:
            ...
            train_split = self.get_overlap_split_from_df(
                all_splits,
                ["train", "annot"],
                training=True,
                use_augmentation=use_augmentation,
            )
            val_split = self.get_overlap_split_from_df(
                all_splits, ["val", "annot"], use_augmentation=use_augmentation
            )
            test_split = self.get_overlap_split_from_df(
                all_splits, ["test", "annot"], use_augmentation=use_augmentation
            )
        ...
        if bool_annot:
            if label == 1:
                patch_annot_path = os.path.join(
                    self.annot_dir, slide_id, slide_id + ".pkl"
                )
